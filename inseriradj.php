	    ...
	    $dbname = "clube";
         ...
         $conn= mysqli_connect ($servername, $user, $password,$dbname ) or die ("problemas na ligação ao mysql");
           ...
           //confirma se o email ja esta a ser utilizado.
			$vUser= "SELECT email FROM treinadores_adjuntos WHERE email ='$email'";
			...
			$vUser= "SELECT idequipa FROM treinadores_adjuntos WHERE idequipa ='$idequipa'";
			$result= mysqli_query($conn,$vUser);
			$resultCheck=mysqli_num_rows($result);
			...
			if ($resultCheck>0) {
				...
				<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=http://localhost/site desporto/treinadores.php'>
					<script type=\"text/javascript\">
						alert(\"Já existe um Treinador Adjunto nessa equipa.\");
					</script>
					";
			}else{
				$sql = "INSERT INTO treinadores_adjuntos (`nome_a`, `dt_nasc`, `email`, `idequipa`) VALUES ('$nome','$data', '$email','$idequipa')";
