 ...
 <link rel="icon" type="image/png" href="images/icons/bola.ico"/>
   ...
   <body class="game_info" data-spy="scroll" data-target=".header">
       ...
       <section id="top">
         ...
         <div class="inner-information-text">
            <div class="container">
               <h3>Treinadores</h3>
               <ul class="breadcrumb">
                 <li><a href="inserirtreinadores.php">Inserir Treinador</a></li>
               </ul>
            </div>
         </div>
      </section>
      <?php 
            ...
            $sql = "SELECT * FROM treinadores inner join equipas where equipas.id_equipa=treinadores.idequipa Order BY id_treinador"; 
            ...
            printf("<tr><tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td><form method='POST' action='alterartreinadores.php'><button value = '%s' name='edit'>Editar</button></form></td><td><form method='POST' action='apagart.php'><button value = '%s' name = 'elim'>Eliminar</button></form></td></tr>", $row["id_treinador"], $row["nome_t"], $row["dt_nasc"],  $row["email"],$row["Escalao"], $row["id_treinador"], $row["id_treinador"]);
        }
            echo "</table>";
             mysqli_free_result($qry);
              ...
              <div class="inner-information-text">
            <div class="container">
               <h3>Treinadores Adjuntos</h3>
               <ul class="breadcrumb">
                 <li><a href="inseriradjunto.php">Inserir Treinadore Adjunto</a></li>
               </ul>
            </div>
         </div>
      </section>
      <?php 
            ...
            $sql = "SELECT * FROM treinadores_adjuntos inner join equipas where equipas.id_equipa=treinadores_adjuntos.idequipa Order BY id_Adjunto"; 
            $qry = mysqli_query($connect, $sql); 
            echo "<table class='table table-bordered table-responsive schedule-table' bgcolor='white'> "; 
            echo "<tr><th>ID</th>
             <th>Nome</th>
            ...
            <th>Escalão</th> </tr"; 
          while ($row = mysqli_fetch_assoc($qry)){
            printf("<tr><tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td><form method='POST' action='alteraradj.php'><button value = '%s' name='edit'>Editar</button></form></td><td><form method='POST' action='apagaradj.php'><button value = '%s' name = 'elim'>Eliminar</button></form></td></tr>", $row["id_Adjunto"], $row["nome_a"], $row["dt_nasc"],  $row["email"],$row["Escalao"], $row["id_Adjunto"], $row["id_Adjunto"]);
        }
            echo "</table>";
             mysqli_free_result($qry);
              mysqli_close($connect); ?>
